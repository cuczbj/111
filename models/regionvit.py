...
class RegionViT(nn.Module):
    def __init__(self, hidden_dim, predicate_dim, box_feature_size,
        ...
        # 修改：只使用 subject 特征，移除 object 和 union 的特征提取器
        # 修改：分类器直接从 box_feature_size 映射到 predicate_dim
        self.rel_classifier = nn.Sequential(*[nn.Linear(box_feature_size, self.hidden_dim),
                                              nn.ReLU(),
    ...
    def forward(self, full_im, bbox_s, bbox_o, predicate):
        ...
        # 修改：只提取 subject 的 ROI 特征
        sub_feature = self.subject_feature_extractor(image_feature, rescaled_bbox_s)

        # 修改：直接使用 subject 特征进行分类
        rel_dists = self.rel_classifier(sub_feature)
